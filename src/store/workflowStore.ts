        ...
        switch (node.type) {
          ...
          case "nanoBanana": {
            ...
            try {
              ...
              // ========== 统一同步模式（支持 google/aabao provider）==========
              // 所有 provider（google、aabao）都使用统一的同步 API：/api/generate
              // 超时 5 分钟，足够处理 4K 生成
              ...
              if (nodeData.seedFixed && nodeData.lastSeed) {
                const cacheKeyData: CacheKeyData = {
                  ...
                  inputs: {
                    ...
                    provider: nodeData.provider || "google", // Use actual provider
                  },
              ...
              if (result.success) {
                // ===== NEW: Handle R2 reference or Base64 =====
                const image = result.imageRef ? null : result.image;
                const imageRef = result.imageRef || null;
                const timestamp = Date.now();
                const imageId = `${timestamp}`;
                ...
                // Auto-save to generations folder if configured (only for Base64)
                ...
                while (pollAttempts < maxPollAttempts) {
                  ...
                  try {
                    ...
                    console.log(`[VIDU-POLL-STORE] Node ${node.id}, pollResult:`, {
                      ...
                      hasImageRef: !!pollResult.imageRef,
                      hasImage: !!pollResult.image,
                      error: pollResult.error,
                      progress: pollResult.progress,
                    });

                    if (pollResult.success && (pollResult.imageRef || pollResult.image)) {
